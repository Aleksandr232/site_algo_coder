...
function quantlab_legal_name(): string
{
    ...
    return $name !== '' ? $name : 'Мельников Александр Анатольевич';
}
...
function quantlab_render_offer_prose(): void
{
    ...
    ?>
          ...
          <p>1.2. Исполнитель — <?= quantlab_h($name) ?>, самозанятый, ИНН <?= quantlab_h($inn) ?>, действующий под коммерческим обозначением AM QuantLab.</p>
          ...
          <p>3.2. Цена на сайте и в счёте — в рублях РФ. Исполнитель — самозанятый. НДС не выделяется.</p>
...
function quantlab_render_privacy_prose(): void
{
    ...
    ?>
          ...
          <p>Оператор — <?= quantlab_h(quantlab_legal_name()) ?>, самозанятый, коммерческое обозначение AM QuantLab, ИНН <?= quantlab_h($inn) ?>. Вопросы по данным: <a href="mailto:<?= quantlab_h($email) ?>"><?= quantlab_h($email) ?></a> или <a href="<?= quantlab_h(quantlab_telegram_url()) ?>" target="_blank" rel="noopener">Telegram</a>.</p>
