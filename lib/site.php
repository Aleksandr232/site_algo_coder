...
function quantlab_legal_name(): string
{
    ...
    return $name !== '' ? $name : 'Индивидуальный предприниматель';
}
...
function quantlab_render_offer_prose(): void
{
    ...
    ?>
          ...
          <p>3.2. Цена на сайте и в счёте — в рублях РФ. Если Исполнитель применяет специальный налоговый режим и не является плательщиком НДС, НДС не выделяется. Иначе НДС указывается по Налоговому кодексу РФ.</p>
          ...
          <p>3.5. После успешной оплаты Заказчик получает кассовый чек на указанный email в соответствии с 54-ФЗ, если это требуется законом и настроено у Исполнителя.</p>
...
function quantlab_render_privacy_prose(): void
{
    ...
    ?>
          ...
          <p>Согласие — отправка формы, сообщение в Telegram или оплата. Исполнение договора — если вы заказываете работу или готового робота. Закон 152-ФЗ и, при оплате, требования 54-ФЗ к кассовому чеку.</p>

          <h2>5. Кому передаём</h2>
          <p>Данные не продаём. Они могут быть доступны сервисам, без которых нельзя принять заявку или платёж: хостинг сайта, почта, Telegram, платёжный агрегатор и оператор фискальных данных. API брокеров и бирж используем только если вы сами даёте ключи для проекта.</p>
